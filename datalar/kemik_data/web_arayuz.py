...
st.set_page_config(
    page_title="Kemik Kırığı Tespit Sistemi",
    layout="wide"
)
...
# Başlık
st.markdown('<div class="main-header">🦴 Kemik Kırığı Tespit Sistemi</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-header">Röntgen Analizi ile Yapay Zeka Destekli Tanı</div>', unsafe_allow_html=True)
...
def main():
    ...
    if model is None:
        st.error("Model yüklenemedi. Lütfen 'kemik_modeli.h5' dosyasının mevcut olduğundan emin olun.")
        return
    
    
    # Sidebar - Bilgi paneli
    with st.sidebar:
        st.header("ℹModel Bilgileri")
        st.info("""
        ...
        **Sınıflar:**
        ...
        st.markdown("""
        1. Kemik röntgeni görselini yükleyin
    ...
    with col1:
        ...
        uploaded_file = st.file_uploader(
            ...
            type=['jpg', 'jpeg', 'png'],
            help="Kemik röntgeni görselini yükleyin"
        )
    ...
    with col2:
        ...
        if uploaded_file is not None:
            with st.spinner('Analiz ediliyor...'):
                ...
                # Sonucu belirle
                if probability > 0.5:
                    diagnosis = "KIRIK TESPİT EDİLDİ (FRACTURED)"
                    confidence = probability * 100
                    color_class = "fractured"
                    emoji = "🔴"
